...
function SimpleHeader(){return <><div className="topbar"><div><span>●</span> Malatya’nın 13 ilçesine hizmet</div><div><a href="mailto:info@cbinsaat.com">✉ info@cbinsaat.com</a></div></div><header className="nav"><Link href="/" className="brand"><span className="brand-bolt">ϟ</span><span><b>CB</b> İNŞAAT<small>ELEKTRİK • MEKANİK • YAPI</small></span></Link><nav><Link href="/">ANA SAYFA</Link><div className="nav-drop"><Link href="/#hizmetler">HİZMETLER <i>⌄</i></Link><div className="mega services-menu"><div><small>ELEKTRİK</small>{services.slice(0,10).map(x=><Link key={x.slug} href={`/hizmetler/${x.slug}`}><span>{x.icon}</span>{x.title}</Link>)}</div><div><small>SU & MEKANİK</small>{services.slice(10,20).map(x=><Link key={x.slug} href={`/hizmetler/${x.slug}`}><span>{x.icon}</span>{x.title}</Link>)}</div><div><small>YAPI & BAKIM</small>{services.slice(20).map(x=><Link key={x.slug} href={`/hizmetler/${x.slug}`}><span>{x.icon}</span>{x.title}</Link>)}</div></div></div><div className="nav-drop"><Link href="/malatya">BÖLGELER <i>⌄</i></Link><div className="mega district-menu"><small>MALATYA’NIN 13 İLÇESİ</small><div>{districts.map(d=><Link href={`/malatya/${d.slug}`} key={d.slug}>{d.name}<span>→</span></Link>)}</div></div></div><Link href="/blog">BLOG</Link><Link href="/teklif">TEKLİF AL</Link></nav><Link className="header-cta" href="/teklif"><small>PROJENİ ANLAT</small><b>Teklif Talep Et</b><span>↗</span></Link></header></>}
